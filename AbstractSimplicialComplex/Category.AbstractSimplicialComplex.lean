...
import AbstractSimplicialComplex.FintypeNECat
import AbstractSimplicialComplex.Equivalence
...
((AbstractSimplicialComplextoPresheaf2.obj K).map f) := by tauto  
/-  ext g 
  rw [AbstractSimplicialComplextoPresheaf_calculation_map2]
  simp only [Prod.swap_obj, Functor.prod_obj, Functor.op_obj, Functor.id_obj, Opposite.unop_op, types_comp_apply]
...
/- The geometric realization: we define it on FintypeNECat by sending S to the standard simplex on S (see FintypeNE.lean), 
extend it to FintypeNECatᵒᵖ ⥤ Type u  by left Kan extension along Yoneda, and then restrict it to AbstractSimplicialComplexCat 
via the reflective functor AbstractSimplicialComplextoPresheaf2. Except that for universe reason, we do the left Kan extension
on the skeleton of FintypeNECat.-/
...
noncomputable def AbstractSimplicialComplex.toTop : AbstractSimplicialComplexCat.{u} ⥤ TopCat.{u} := 
AbstractSimplicialComplextoPresheaf2 ⋙ (Equivalence.FunctorCategory _ _ (Type u) FintypeNECat.Skeleton.equivalence.op).inverse ⋙ 
PresheafSkeleton.toTop 
